...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    const SidebarContent = () => (
        <>
            <div className="p-4 md:p-6 border-b border-surface-800">
                <h2 className="text-lg md:text-xl font-display font-bold text-primary-400">Admin Panel</h2>
                ...
                {menuItems.map((item) => {
                    ...
                    return (
                        <Link
                            key={item.name}
                            href={item.href}
                            className={`flex items-center gap-3 p-2.5 md:p-3 rounded-lg transition-all duration-200 ${
                                isActive
            ...
            <div className="p-3 md:p-4 border-t border-surface-800 space-y-3">
                <div className="px-2 md:px-3">
                    <p className="text-xs text-surface-500 truncate">{user.email}</p>
                </div>
                <button
                    onClick={handleSignOut}
                    className="flex items-center gap-3 p-2.5 md:p-3 w-full rounded-lg text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-colors"
                >
    ...
    return (
        <div className="min-h-screen bg-surface-50 flex">
            ...
            <aside className={`
                fixed h-full bg-surface-900 text-white flex flex-col shadow-2xl z-50
            ...
            <main className="flex-1 lg:ml-64 min-w-0">
                ...
                <div className="lg:hidden sticky top-0 z-30 bg-white border-b border-surface-200 px-4 py-3 flex items-center gap-3 shadow-sm">
                    <button
                        onClick={() => setSidebarOpen(true)}
                        className="p-2 rounded-lg bg-surface-100 text-surface-700 hover:bg-surface-200 transition-colors"
                    >
                        <Menu size={20} />
                    </button>
                    <h1 className="font-display font-bold text-surface-900 text-base flex items-center gap-1.5">
                        Admin <ChevronRight size={16} className="text-surface-400" />
                        <span className="text-primary-600">{currentTitle}</span>
                    </h1>
                    <div className="ml-auto">
                        <span className="flex items-center gap-1.5 px-2 py-1 bg-green-100 text-green-700 rounded-full text-[10px] font-bold border border-green-200 uppercase tracking-wider">
                            Online
                        </span>
                    </div>
                </div>
